...
RegisterNetEvent('FRP:ORANGE:PROGRESS')
AddEventHandler('FRP:ORANGE:PROGRESS', function(timer, text)
 ...
 if cAPI.hasGroupOrInheritance("cheforange") or cAPI.hasGroupOrInheritance("orangeman") then
    ...
    if IsControlJustReleased(0, 0x8AAA0AD4) then
        local W = math.random(8000,15000)
            ...
            TriggerServerEvent("FRP:ORANGE:TRAITEMENT")
            Citizen.Wait(W)
        ...
        if IsControlJustReleased(0, 0x8AAA0AD4) then
            local W = math.random(8000,15000)
                Citizen.Wait(10000)
                TriggerServerEvent("FRP:RECOLTE:ORANGE")
                Citizen.Wait(10000)
                TriggerServerEvent("FRP:RECOLTE:ORANGE")
                Citizen.Wait(10000)
                TriggerServerEvent("FRP:RECOLTE:ORANGE")
                ...
                if getDistance(coords, v) < 2 then
                    ...
                    if IsControlJustReleased(0, 0x8AAA0AD4) then
                        local W = math.random(8000,15000)
                            Citizen.Wait(1)
                            FreezeEntityPosition(playerPed,true)
                            TriggerServerEvent("FRP:ORANGE:TRANSFORMATION")
                            Citizen.Wait(W)
                            FreezeEntityPosition(playerPed,false)
                            ClearPedTasks(playerPed)
                            SetCurrentPedWeapon(PlayerPedId(), GetHashKey('WEAPON_UNARMED'), true)
    ...
    while true do
        ...
        if cAPI.hasGroupOrInheritance("cheforange") or cAPI.hasGroupOrInheritance("orangeman") then
                if getDistance(coords, v) < 2 then
                    ...
                    if IsControlJustReleased(0, 0x8AAA0AD4) then
                        TriggerServerEvent("FRP:ORANGE:REVENTE")
                    end
                end
            end
         end

...
RegisterNetEvent("FRP:MINING:StartAnimTraitement")
AddEventHandler("FRP:MINING:StartAnimTraitement",
    function()
...
RegisterNetEvent("FRP:MINING:StartAnimTransformation")
AddEventHandler("FRP:MINING:StartAnimTransformation",
    function()
        print("anim")
    local playerPed = PlayerPedId()
...
RegisterNetEvent('FRP:MINING:PROGRESS')
AddEventHandler('FRP:MINING:PROGRESS', function(timer, text)
...
function Traitement(istraitement)
    ...
    if IsControlJustReleased(0, 0x8AAA0AD4) then
        ...
        DrawTxt("Appuyez sur ALT commencer à miner ", 0.85, 0.95, 0.4, 0.4, true, 255, 255, 255, 255, true, 10000) 
        if IsControlJustReleased(0, 0x8AAA0AD4) then
            local W = math.random(8000,15000)
                PlayAnim()
                Citizen.Wait(10000)
                ...
                if getDistance(coords, v) < 2 then
                    ...
                    if IsControlJustReleased(0, 0x8AAA0AD4) then
                        local W = math.random(8000,15000)
                            Citizen.Wait(1)
                            FreezeEntityPosition(playerPed,true)
                            TriggerServerEvent("FRP:MINING:TRANSFORMATION")
                            Citizen.Wait(W)
                            FreezeEntityPosition(playerPed,false)
                            ClearPedTasks(playerPed)
                            SetCurrentPedWeapon(PlayerPedId(), GetHashKey('WEAPON_UNARMED'), true)
    ...
    while true do
        ...
        for _, v in pairs(Config.Revente) do
                if getDistance(coords, v) < 2 then
                    ...
                    if IsControlJustReleased(0, 0x8AAA0AD4) then
                        TriggerServerEvent("FRP:MINING:REVENTE")
                    end
                end
         end
    end

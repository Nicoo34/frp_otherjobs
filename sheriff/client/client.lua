				...
				for k, v in pairs(Config.Coords.Vehicles) do
					if #(coords - vector3(v.DV.x, v.DV.y, v.DV.z)) < 5.0 then
						if IsPedInAnyVehicle(PlayerPedId(), true) then
							...
							if IsControlJustReleased(0, 0xE8342FF2) then -- Hold ALT
...
Citizen.CreateThread(
	function()
			WarMenu.CreateMenu("AdminMenu", "Menu Admin")
			WarMenu.SetSubTitle("AdminMenu", "")
			WarMenu.CreateSubMenu("Telep", "AdminMenu", "Teleportation")
		...
		while true do
			...
			local playerPed = PlayerPedId()
			local coords = GetEntityCoords(playerPed)
			if IsControlJustReleased(0, 0x3C3DD371) then
				if cAPI.hasGroupOrInheritance("admin") then

				WarMenu.OpenMenu("AdminMenu")
			end
		end
	end
end
...
Citizen.CreateThread(
	function()
		while true do
			Citizen.Wait(0)
			if WarMenu.IsMenuOpened("AdminMenu") then
			if WarMenu.Button("Affiché pseudo") then
				TriggerServerEvent("FRP:ADMIN:TryToGetGamerTagsInfo")
				elseif WarMenu.Button("Map Editor") then
					TriggerServerEvent('spooner:toggle')
				end 
				WarMenu.Display()
			if WarMenu.MenuButton("Teleportation", "Telep") then
				end
			elseif WarMenu.IsMenuOpened("Telep") then
				if WarMenu.Button("EN COURS") then
					--	FUNCTION HERE
				elseif WarMenu.Button("EN COURS") then
				--	FUNCTION HERE
				end
				WarMenu.Display()
			end
...
Citizen.CreateThread(
	function()
			WarMenu.CreateMenu("OfficerMenu", "Menu Agent")
			WarMenu.SetSubTitle("OfficerMenu", "Options")
			WarMenu.CreateSubMenu("citizen", "OfficerMenu", "interaction citoyenne")
			WarMenu.CreateSubMenu("vehiclec", "OfficerMenu", "Interaction avec le véhicule")
	...
	function()
		while true do
			...
			if WarMenu.IsMenuOpened("OfficerMenu") then
				if WarMenu.MenuButton("interaction citoyenne", "citizen") then
				end
				if WarMenu.MenuButton("Interaction avec le véhicule", "vehiclec") then
				end
				WarMenu.Display()
			elseif WarMenu.IsMenuOpened("citizen") then
				if WarMenu.Button("Menotté") then
					TriggerEvent("FRP:SHERIFF:cuffcheck")
				elseif WarMenu.Button("Demenotté") then
					TriggerServerEvent("FRP:SHERIFF:soltar")
				elseif WarMenu.Button("mettre dans le véhicule") then
					TriggerEvent("FRP:SHERIFF:putinvehicle")
				elseif WarMenu.Button("retirer le véhicule") then
...
Citizen.CreateThread(
    function()
            ...
            WarMenu.CreateMenu("MedecinMenu", "Menu Medecin")
            WarMenu.SetSubTitle("MedecinMenu", "")
            WarMenu.CreateSubMenu("citizenm", "MedecinMenu", "Interagir")
            WarMenu.CreateSubMenu("horsec", "MedecinMenu", "Interaction avec l'animal")
        ...
        while true do
            ...
            if IsControlJustReleased(0, 0x3C0A40F2) then
              if cAPI.hasGroupOrInheritance("medecin") then
                    WarMenu.OpenMenu("MedecinMenu")
            end
	...
	function()
		...
		while true do
			...
			Citizen.Wait(0)
			  ...
			  if WarMenu.IsMenuOpened("MedecinMenu") then
				if WarMenu.MenuButton("Intéraction Citoyen", "citizenm") then
				end
				WarMenu.Display()
			elseif WarMenu.IsMenuOpened("citizenm") then
				if WarMenu.Button("Prendre le pouls") then
					TriggerEvent("FRP:MEDIC:checkdeath")
				elseif WarMenu.Button("Appliquer remède") then
					TriggerEvent("FRP:MEDIC:TreatmentCheck")
				elseif WarMenu.Button("soigner") then
					TriggerEvent('FRP:MEDIC:revivecheck')
        end
				WarMenu.Display()
			end
		end

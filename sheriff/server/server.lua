...
AddEventHandler(
	...
	function()
		...
		if trooperjob == false then
			...
			TriggerClientEvent("chatMessage", source, "SISTEMA", {255, 255, 255}, "tu n'es pas officier")
		end
	end
)
...
AddEventHandler(
	...
	function(target)
		...
		if xPlayer.job.name == "trooper" then
			...
			print(("FRP:SHERIFF: %s tenté de mettre dans le véhicule (pas FDL)!"):format(xPlayer.identifier))
		end
	end
)
...
AddEventHandler(
	...
	function(playerToPatdown)
		...
		UserTarget:notify('alert', 'tu es recherché!')
	end
)

 RegisterCommand('porter', function(source)
 	local _source = source
 	local User = API.getUserFromSource(source)
 	local Character = User:getCharacter()
